...
-- | Run bot application
application :: (MT m, IBot pointer init _update) =>  pointer -> MVar Bool -> m ()
application pointer userExitMVar= do
    Log.setSettings Color.Blue True "application"
    uidFromFile <- Cache.getUpdateIdFromFile
    init <- Bot.getInit pointer userExitMVar
    if uidFromFile then do
        uid <- Cache.getUpdateId
        Log.infoM $ template "Received updateId from file: {0}" [show uid]
        longPolling pointer (Bot.setUpdateId init uid) userExitMVar -- updateId from the request, overwrite the one from the file
    else do
        longPolling pointer init userExitMVar

-- | Long polling loop
longPolling :: (MT m, IBot pointer init update) => pointer -> init -> MVar Bool -> m ()
longPolling pointer init userExitMVar = do
    ...
    if exit 
        ...
        else longPolling  pointer newInit userExitMVar
